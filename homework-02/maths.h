...
// readInput needs to be called before the first invocation of random_float
// otherwise rand() hasn't been seeded yet
float random_float() {
    static thread_local std::mt19937 generator(get_thread_safe_rand());
...
Ray get_camera_ray(const Camera& cam, float u, float v) { 
    return Ray(cam.origin, cam.lower_left_corner + cam.horizontal * u + cam.vertical * v - cam.origin); 
}